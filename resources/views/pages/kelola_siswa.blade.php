    ...
    <main class="main container" id="main"> 
        <div class="container mt-5">
            <div class="d-flex justify-content-end">
                <form class="d-flex me-3 mb-0" action="{{ route('kelola_siswa.siswa') }}" method="GET">
                    <input type="text" name="cari" placeholder="Cari Nama akun ..." class="form-control me-2">
                    <button type="submit" class="btn btn-primary">Cari</button>
                </form> 
            <a href="{{ route('kelola_siswa.tambah')}}" class="btn btn-success" >+ Tambah pengguna</a>
        </div>
    ...
    @endif
            <table class="table table-stripped table-bordered mt-3 text-center">
                ...
                {{-- Nomor urut = (2 - 1) x 10 + (5 + 1) = 1 x 10 + 6 = 16 --}}
                    ...
                    <td>{{$bagian['tingkat']}}</td>
                    <td class="d-flex justify-content-center">
                        <a href="{{ route('kelola_siswa.ubah',$bagian['id'])}}" class="btn btn-primary me-2">Edit</a>
...
function showModalDelete(id, name) {
    ...
    $("#nama").text(name);
    let url = "{{ route('kelola_siswa.hapus', ':id')}}";
